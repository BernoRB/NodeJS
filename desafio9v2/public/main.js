...
socket.on('mensajes', (mensajes) => {
  const autorSchema = new normalizr.schema.Entity('autors')
  const messageSchema = new normalizr.schema.Entity('mensajes',
    { autor: autorSchema },
    { idAttribute: '_id' })
  const global = new normalizr.schema.Entity('global',
    { messages: [messageSchema] } )
  const chat = normalizr.denormalize(mensajes.result, global, mensajes.entities)

  renderMsgs(chat.messages)
  
  porcComp(chat, mensajes)

})

const porcComp = async (denorm, norm) => {
  const porcentage = Math.floor(100 - (JSON.stringify(norm).length * 100) / JSON.stringify(denorm).length)
